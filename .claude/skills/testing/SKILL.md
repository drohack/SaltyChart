...
**A silently degraded dependency is the hardest thing here to test, and the
cheapest thing to get wrong.** skyhook began answering 400 to axios's default
`User-Agent`; `skyhookShow`'s bare `catch` turned every failure into an empty
result and **cached it**, so the entire TVDB evidence tier was dead - the
season-premiere rung could not fire, `hasUndatedFutureSeason` was always false,
and the cross-provider merge had no id to merge on. Nothing failed: "could not
ask" and "no schedule exists" were the same value. The unit tests that now hold
it do not test the header (no network offline); they test the part that made the
outage permanent - **a failed lookup is not cached**, a successful one is. When
you wrap an upstream call in a `catch`, the test to write is "does a failure
stay distinguishable from an empty answer, and does it get retried".
...
| when | what | cost |
|---|---|---|
| every push to master | `run_all.py --skip-burned-in` | ~6.5 min (re-measured 2026-09-21, 23 checks) |
...
runs once, immediately before a push. It takes **~6.5 min** with
`--skip-burned-in` (re-measured 2026-09-21 at 23 checks: a 35 s parallel
phase plus 353 s of sequential checks) and `test_player`
...
| `test_ui_interactions.py` | 29 flows: nine button-click smoke (login -> modal Escape), then one guard per silent-failure class found by the audits and exploratory passes - Compare ranks checked against seeded orders, no API key in /admin's DOM, `unknown`/`notAired` never hide, share-image has real content, per-section progressive loading, a visible unreachable-library / hung-backend / failed-hide-rollback, the /admin/matching resolver-accept + match-dropdown + Run-sweep-button contract, check-batch chunking, in-band translation errors, the phone sidebar default (and that a dismissal of it is remembered), guest options, an oversized wheel image warning instead of wedging the page, a theme choice surviving signup with no wrong-theme first paint, the viewer's library picker (its write stubbed - a real pick rewrites identity for everyone), and **/admin/subtitles' Run-now button** reaching `POST /api/translate/batch` and disabling itself - with alerts switched off around it, because a batch exiting non-zero mails the owner and a test that mails a human on every run trains them to ignore it. Seeds from live season data. Every flow's history and the trap it was watched to fail on: the file's docstring The newest guards **/admin/sonarr**, which had no browser coverage at all: every defect in it - a stray `<td>` putting 8 cells in a 7-column table, a badge wrapping inside its pill, an Include button overlapping a clipped badge, and a headline reading "0 will be added" when Sonarr could not be reached - was found by a person looking, with tsc, svelte-check and the API tests all green. It asserts the SHAPES of those four: cells match the colgroup, every row table shares one column layout, nothing is clipped or wrapped, and the headline never reports "couldn't ask" as zero (checked in both directions so the step is never vacuous). |
| `test_subtitle_paths.py` | Subtitle Paths B/C/D - YouTube CC, Whisper overlay, CC toggle persistence. Path B also pins that the **fullscreen button exists and works while YouTube CC is active** and that Escape in fullscreen leaves the modal alone (it exits via the button, because a synthetic Escape is not the browser's leave-fullscreen gesture) - the fullscreen button was once wrapped in the `!hasEnglishSubs` gate with `allowfullscreen` gone from the iframe, so YouTube-CC trailers had no fullscreen at all. Its mutation row (`T_SUBS`) hides the button for exactly those viewers. The overlay is found by `.sc-subtitle`; the layout-class selector it replaced matched nothing after the cue was re-laid-out, and Path C failed on a clean build |
...
| `test_sonarr.py` | 9 steps on the Sonarr auto-add. This feature WRITES to someone's media server, so the assertions are about the **contract** and above all the **guards**, never which shows are in it. Step 1: **every route must 401 unauthenticated** - there is no public route at all now (the Custom List's `GET /list` was the one, and it went with the list). Step 5 is the one that matters most: it **pauses explicitly**, runs a real `POST /push`, and asserts the `SonarrPush` rows are byte-identical before and after - reading the rows, not the response body, because a guard that wrote first and reported `paused` afterwards would sail through a body check (a hide-rollback test here once checked the server, which is trivially correct when every write fails). It restores the previous setting, and a crash leaves it PAUSED, the safe direction. Steps 2-4: plan shape, **positive integer** tvdbIds (`Identity.tvdbId` is a string, so a missing `Number()` ships a quoted id the lookup misses), **no duplicate tvdbIds** (several AniList ids map to one TVDB id and `resolveIdentity` does not dedupe - no live collisions when measured, so only a prospective assertion can catch it), and query params that 400 rather than silently falling back. Steps 6-7 cross-check the filter against live season data and prove the **air window** is what bounds the plan rather than the identity gate - without it a broken identity lookup looks exactly like a working air-date filter. Step 8: **`/report` must degrade rather than fail** - with Sonarr unreachable it still returns the whole candidate side with `sonarr.observed: false`, since an outage is exactly when someone opens the page - plus `history.pushed` may only count rows whose status is `pushed`, which is written solely on a 201 from Sonarr. Step 9: every row graded, zero `weak` among candidates, and an unverified include refused with 409. **A successful add is not covered** - it needs write credentials and changes a real library, so it is a manual step (push one with `cap: 1`, check Sonarr's UI, push again and confirm the second run is a no-op). Auto-skips (exit 0) on a cold season cache - these endpoints are forbidden from fetching one - and skips everything after step 1 if it cannot mint an admin token |
...
| `test_account_security.py` | The account and admin guards, on a backend it boots itself with an **empty** database. Not steps in `test_api_negative.py`, and the reason is load-bearing: these assertions attempt destructive admin actions, which are refused while the guards hold - and SUCCEED under a mutation run. Against the dev server that meant the real admin's password reset to "taken_over", its email cleared, and a fixture left promoted; the audit reverts source, never data, so nothing put it back. An empty DB is also the only place the `[SETUP]` first-run claim flow exists at all. Covers: the claim code is printed, required, and single-use; signing up confers nothing; an admin with no verified email has no reset path by either door; promotion needs a verified email on the target; a verified address closes the open reset while an address-less account keeps it; an admin's email cannot be cleared; the last admin survives demote and delete; a password change kills existing tokens; and an abandoned email change keeps the confirmed address (the window that once left an admin with no reset path mid-change). **Always rebuilds `dist/`** - reusing it graded a pre-fix build and reported a bug that was already fixed - and **refuses to start on an occupied port**, after a leftover backend answered every request from a different database |
...
| `test_yt_guard.py` | The enforced YouTube budget, offline and sub-second, with state redirected to a temp file so the test can neither spend nor reset the real budget. Every matcher decision is pinned because every one was wrong once and found by blocking a person mid-work: the guard's first version blocked `curl raw.githubusercontent.com/yt-dlp/...`, blocked `echo "yt-dlp wiki FAQ"`, silently could not gate `ytimg.com`, and could not see `curl .../api/translate/stream` - the very loop it was built to stop. Also the four budget gates at exactly their caps, cooldown on a block, strike-based tightening (halve caps, double cooldown, floors, week-old strikes expire), `--clear` keeping strike history, and that the **live** `.claude/settings.json` pre-filter reaches every invocation sample - the first `--selftest` compared samples against this file's own token list, a tautology that stayed green while four patterns were unreachable, so a fixture of the grep as it shipped now pins that it could NOT reach `local_translate.py`, and the live check FAILS on a checkout whose grep is not widened (that is the point). Three mutation rows: the off-by-one loosening of the burst cap, the return to substring matching, and `prefilter_reaches` answering yes to everything |
| `test_status_page.py` | `/api/status` - the upstream service status page. Forces a real probe of every configured service, so it is also the only check that would notice a probe query going stale (the AniList one shipped selecting only `pageInfo` and got a 400). Every route 401s unauthenticated and 403s for a signed-up non-admin; `/report` names every registered service and gives each a server-decided state; **a service that is not set up reads `notConfigured`, never `down`**, and after a forced probe nothing probeable is still `unknown`; the alert settings round-trip, with a malformed address dropped rather than stored. Lives in the sequential group and backs up both AppConfig keys |
...
| `test_download_hold.py` | The production twin of the guard. After the daemon reports a `botwall` failure, `/stream` must refuse a cache miss with the friendly message and a `holdUntil` - without asking the daemon for anything - and must record nothing, because a refusal is not an attempt; `/check-batch` must queue nothing and say so in `X-Check-Queue: held` (the header exists so the assertion is non-vacuous with no daemon running - the mutant's `no-daemon` is how it was watched to fail); `/check` must answer from cache only with `hasEnglish: null` and write no row. Backs up `AppConfig.subtitleDownloadHealth`, injects a bot wall "just now", requests a never-existing id, asserts, restores in a `finally`. Runs in `run_all`'s **sequential** group because it mutates shared backend state. Three route rows (`hold.hold && !hold.hold` on stream and check - type-checks where `false &&` would narrow - and the inverted hold on check-batch), each costing at most one harmless YouTube "unavailable". The asymmetry that a plain 403 must NOT hold is a `T_UNIT` row on `downloadHealth.test.ts` |
| `test_run_verdict.py` | Offline. Both batch scripts used to count download errors, print them per season, and fall off the end of `main()` with exit 0 - the Sunday GPU run logged `FALL 2026: 3 translated, 46 errors` four Sundays running while Task Scheduler showed `lastResult=0x0`. `run_verdict` is the missing half; this pins the case that shipped (46 of 49 -> exit 2 and the stale-yt-dlp remedy in the final `Done:` line), what must NOT fail a run (two private trailers, 3 of 49, nothing to do, 2 of 3 under `RUN_FAIL_MIN`), both threshold edges (3 of 6 exactly at the ratio passes; 4 of 6 fails), the bot-wall abort code 3, `classify_error`'s phrase-over-status rule, and that `yt_guard.BLOCK_SIGNS` is the imported container tuple. Also: the **three-valued CC check** - `classify_check_exception` on exception classes named like `youtube_transcript_api`'s (defined in the test, so no package needed), then `check_subtitles` itself driven through a fake module that blocks (a timeout is `null`, never `false`) or raises `IpBlocked`, plus a SKIP-not-PASS check that `DEFINITIVE_NO_CC` names exist in the installed package; **`MODEL_RANK` parity** with `lib/subtitleReport.ts` through ts-node (node missing is a FAIL, the gate needs it) and an AST check that neither batch script assigns its own copy; and `ensure_ytdlp_current` retrying exactly once without `--break-system-packages` when pip says `no such option` (monkeypatched `subprocess.run`, restored in `finally`). Four mutation rows: the impossible comparison that makes a run unable to fail, the classifier calling every failure a definitive no, `large-v3-split` demoted to tie plain `large-v3`, and the retry removed |
| `test_local_run_report.py` | The Sunday run's self-report route. The server could not see that job at all - a failed run uploads nothing, so it looked like a quiet week - and `local_translate.py` now ends by posting its verdict to `POST /local-run`. Pins the gates (401, 403 for a throwaway non-admin, 400 on a malformed body), a real failed verdict round-tripping into `/report.schedule.lastLocalRun` with counts and breakdown, and the `reportedAt` stamp. Mutation row drops `requireAdmin` from the route: the admin page repeats whatever is stored, so an open route lets any account paint a failed month green |
...
Final line on success: `Pre-deploy: 24/24 passed - ready to build` (23/23 with
