...
| `test_sonarr.py` | 8 steps on the Sonarr Custom List. Sonarr treats this list as authoritative and cannot tell a short one from a complete one, so the assertions are about the **contract**, never which shows are in it: a **bare array** (an object parses as zero series and reads as a list that adds nothing), items of exactly `{title, tvdbId}` with a **positive integer** id (`Identity.tvdbId` is a string, so a missing `Number()` ships a quoted id Sonarr rejects), **no duplicate tvdbIds** (several AniList ids map to one TVDB id and `resolveIdentity` does not dedupe - there were no live collisions when measured, so only a prospective assertion can catch it), query params that 400 rather than silently falling back, and a live cross-check that nothing adult / `MOVIE` / PREQUEL-having leaks through. Step 6 proves the **air window** is what bounds the list rather than the identity gate - without it a broken identity lookup looks exactly like a working air-date filter. The `identityReady()` 503 is **not** covered: racing it needs a restart, so it is verified by hand. Steps 7-8 cover the admin half: **every admin route must 401 unauthenticated** (this router serves exactly ONE public route, and `/report` carries what Sonarr holds), and **`/report` must degrade rather than fail** - with Sonarr unreachable it still returns the whole proposal side with `sonarr.reachable: false`, since an outage is exactly when someone opens the page. Step 8 also asserts `/report` and `/list` propose the same count, so the page and Sonarr can never be looking at different lists. Auto-skips (exit 0) on a cold season cache - this endpoint is forbidden from fetching one - and step 8 skips if it cannot mint an admin token |
