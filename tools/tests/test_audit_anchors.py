...
def check_no_op_rows(rows) -> int:
    """A row whose `replace` equals its `find` changes nothing when applied.

    It can never fail, so the audit's verdict on it is meaningless in BOTH
    directions - CAUGHT would be a lie and SURVIVED reads as a coverage hole
    that isn't one. The anchor check cannot see it, because the anchor resolves
    perfectly well.

    This exists because it happened: a row meant to delete the `deferred` branch
    of `planPushRun` was generated by slicing source between two markers, and the
    end marker (`    }`) matched inside the start marker (`    } else {`). The
    row shipped as a literal no-op, resolved cleanly, and reported SURVIVED -
    which sent someone hunting for a bug in the code it was supposed to guard.
    """
    noop = [f"  [{i}] {m.name}" for i, m in enumerate(rows, 1)
            if m.find.strip() == m.replace.strip()]
    if not noop:
        print("Done: no mutation row is a no-op", flush=True)
        return 0
    print(f"[anchors] FAIL - {len(noop)} row(s) change nothing when applied:", flush=True)
    for n in noop:
        print(n, flush=True)
    print("[anchors] find == replace, so the row's test is never exercised and "
          "neither CAUGHT nor SURVIVED means anything.", flush=True)
    return 1
