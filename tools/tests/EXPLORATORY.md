...
- **Your own script can create the bug you then report.** A loop that clicked
  "Mark as watched" and then ✕ faster than a human left rows in a state a real
...
- **Go at human speed or you will invent bugs.** Clicking a popup button and
  then its ✕ 1.8 s apart produced a DB state no person can reach. Several
...
**8. On a phone, 390×844,** then 768. Tap targets under ~44 px, modals
...
| Escape / close | `<svelte:window on:keydown>` in `AnimeGridTranslate` + `Escape` added to Randomize's existing window handler; a real ✕ button on the trailer modal |
...
- **The My List sidebar opened over the whole screen on every phone page load,
  for logged-in users.** At 375×667 with full mobile emulation the `<aside>` is
  375×667 at (0,0), opaque, and 25/25 sampled viewport points land on it —
  `collapsed` in `WatchListSidebar.svelte` initialised to `false` with no width
  check and no persistence. One tap on the 24×64 px chevron dismisses it and the page is
...
- **Escape closed neither the trailer modal nor the Randomize pop-up.** The
  ...
  **Why the suite missed it, and the lesson:** `test_ui_interactions` step 9 was
  ...
  asserts the ✕ button exists. **When a test offers an alternative path on
...
- **The id tier is healthy; fuzzy matching is dangerous exactly where the id tier
  ...
  **The fix is upstream of the matcher**: a series that has not aired cannot be
  ...
  |---|---|---|
  | FALL 2026 | 83 | **83 × `NOT_YET_RELEASED`** |
...
- **Player subtitle + quality switching** — subtitle menu correctly defaulted to
  ...
  1280×720 to **854×480** and resumed at 28.4 s. Three `playback/stop` calls for
