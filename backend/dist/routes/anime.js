...
const memory = new lru_cache_1.LRUCache({ max: 20, ttl: 1000 * 60 * 60 });
router.get('/', async (req, res) => {
    const { season, year, format } = req.query;
    if (!season || !year) {
  ...
  `;
    try {
        ...
        // strings such as 'TV'.
        const cacheKeyFormat = format ? format.toUpperCase() : '';
        // -------------------------------------------------------------------
        // Cache lookup
        // -------------------------------------------------------------------
        ...
        const cached = await db_1.default.$queryRawUnsafe(`SELECT data, strftime('%s', updatedAt) AS updatedEpoch
       FROM   "SeasonCache"
       WHERE  season = ?
       AND    year   = ?
       AND    format = ?
       LIMIT  1`, season.toUpperCase(), Number(year), cacheKeyFormat);
        ...
        while (hasNext) {
            ...
            while (true) {
                try {
                    response = await axios_1.default.post('https://graphql.anilist.co', {
                        query,
                        variables: {
                            page,
                            perPage,
        ...
        // Save/replace cache
        await db_1.default.$executeRawUnsafe(`INSERT OR REPLACE INTO "SeasonCache" (season, year, format, data, updatedAt)
       VALUES (?, ?, ?, ?, datetime('now'))`, season.toUpperCase(), Number(year), cacheKeyFormat, JSON.stringify(allMedia));
